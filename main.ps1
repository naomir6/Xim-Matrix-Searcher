...
foreach ($RegEntry in $RegPaths) {
    try {
        if (Test-Path $RegEntry.Path) {
            $FoundMatch = $false
            Get-ChildItem -Path $RegEntry.Path -ErrorAction SilentlyContinue | ForEach-Object {
                if ($_.PSChildName -match $VendorID -and $_.PSChildName -match $DeviceID) {
                    $FoundMatch = $true
                    Add-Detection "Registry - $($RegEntry.Name)" "$($_.PSChildName)"
                    Write-Log ">>> DETECTED in $($RegEntry.Name): $($_.PSChildName)"
                }
...
try {
    ...
    $CompositeDevices = $AllUSB | Where-Object {
        $_.InstanceId -match "USB\\VID_$VendorID"
    }
    
    if ($CompositeDevices) {
        foreach ($Device in $CompositeDevices) {
            if ($Device.InstanceId -match $DeviceID) {
                Add-Detection "USB Composite" $Device.FriendlyName
                Write-Log ">>> DETECTED: $($Device.FriendlyName)"
            }
        }
    }
...
foreach ($Class in $DeviceClasses) {
    try {
        $Devices = Get-PnpDevice -Class $Class -ErrorAction SilentlyContinue | Where-Object {
            $_.InstanceId -match $VendorID -and $_.InstanceId -match $DeviceID
        }
...
try {
    $LogitechDevices = Get-PnpDevice -ErrorAction SilentlyContinue | Where-Object {
        $_.InstanceId -match "046D" -and $_.InstanceId -match $DeviceID
    }
    ...
    if ($LogitechDevices) {
        ...
        Write-Log "    No exact matches found"
    }
} catch {
    Write-Log "    ERROR: $($_.Exception.Message)"
}
