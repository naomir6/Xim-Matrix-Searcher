...
Write-Log "[1/14] Scanning PnP Devices..." -NoTimestamp
try {
    ...
    if ($AllDevices) {
        ...
        Write-Log "    Found $($AllUSBDevices.Count) USB device(s) - no target match"
...
Write-Log "[2/14] Scanning Device Properties..." -NoTimestamp
try {
...
Write-Log "[3/14] Scanning Registry..." -NoTimestamp
$RegPaths = @(
    ...
    try {
        if (Test-Path $RegEntry.Path) {
            ...
            if ($RegEntry.Name -like "*USB*") {
                Write-Log "    $($RegEntry.Name): $($AllKeys.Count) entries"
                foreach ($Key in $AllKeys) {
                    Write-Log "      $($Key.PSChildName)"
...
Write-Log "[4/14] Scanning Mounted Devices..." -NoTimestamp
try {
...
Write-Log "[5/14] Scanning Driver Store..." -NoTimestamp
...
# Section 6: INF Cache
Write-Log "[6/14] Scanning INF Cache..." -NoTimestamp
...
Write-Log "[7/14] Scanning SetupAPI Logs..." -NoTimestamp
$SetupLogs = @(
...
Write-Log "[8/14] Scanning Event Logs..." -NoTimestamp
$EventLogs = @(
...
Write-Log "[9/14] Scanning System Events..." -NoTimestamp
try {
...
Write-Log "[10/14] Scanning Hardware IDs..." -NoTimestamp
...
Write-Log "[11/14] Scanning WMI..." -NoTimestamp
try {
...
Write-Log "[12/14] Scanning USB Composites..." -NoTimestamp
try {
    ...
    Write-Log "    Found $($CompositeDevices.Count) USB/Composite device(s)"
...
Write-Log "[13/14] Scanning Device Installation..." -NoTimestamp
...
Write-Log "[14/14] Scanning Device Parameters..." -NoTimestamp
